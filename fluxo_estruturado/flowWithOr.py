...
class State(BaseModel):
    ...
    log: str = "" # Log do fluxo
class FluxoEstruturadoWithOr(Flow[State]):
    ...
    # Mé todo de log que escuta todas as etapas utilizando or_
    @listen(or_(etapa_inicial, etapa_intermedia, estapa_final))
    def registrar_log(self):
        estapa_atual = self.state.etapas_concluidas
        self.state.log += f"\nEtapa {estapa_atual} concluída. Estado: {self.state.info}"
        return self.state
    
processo = FluxoEstruturadoWithOr()
estado = processo.kickoff()

print(f"Estado final: {estado.info}")
print(f"Log final: {estado.log}")
